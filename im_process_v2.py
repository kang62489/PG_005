...
def detrend_parallel(data: np.ndarray, window_size: int = 11) -> np.ndarray:
...
@jit(nopython=True)
def spatial_average(frame: np.ndarray, roi_size: int) -> np.ndarray:
...
@jit(nopython=True, parallel=True)
def process_frames(data: np.ndarray, roi_size: int) -> np.ndarray:
...
def validate_input(filepath: str, roi_size: int) -> None:
    """Validate input parameters"""
    if not Path(filepath).exists():
        err_msg = f"Input file not found: {filepath}"
    ...
    try:
        ...
        err_msg = f"Invalid TIFF file: {e!s}"
        console.print(f"[bold red]{err_msg}")

    if len(shape) != required_dims:
        err_msg = "Input must be a 3D TIFF stack"
        raise ValueError(err_msg)

    # Verify ROI size is valid
    if shape[1] % roi_size != 0 or shape[2] % roi_size != 0:
        err_msg = f"ROI size ({roi_size}) must be a factor of image dimensions ({shape[1]}, {shape[2]})"
        raise ValueError(err_msg)


def process_file(filepath: str, roi_size: int, config: dict) -> None:
...
def main() -> None:
    ...
    files = Path.glob(args.input) if args.input else ["raw_images/2025_06_11-0013.tif"]
