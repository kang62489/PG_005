...
def process_on_gpu(image_stack, roi_size, window_size=100):
    ...
    detrend_kernel[blocks_per_grid, threads_per_block](
        gpu_input,
        gpu_output,
        window_size,
    )
    ...
    for frame_idx in range(n_frames):
        ...
        frame_reshaped = detrended_stack[
            frame_idx,
            :height_adjusted,
            :width_adjusted,
        ].reshape(
            height_adjusted // roi_size,
            roi_size,
            width_adjusted // roi_size,
            roi_size,
        )
        ...
        for i in range(height_adjusted // roi_size):
            for j in range(width_adjusted // roi_size):
                ...
                x_start = j * roi_size
                x_end = (j + 1) * roi_size
                averaged_stack[frame_idx, y_start:y_end, x_start:x_end] = roi_means[
                    i,
                    j,
                ]
...
def process_on_cpu(
    ...
    def detrend_parallel(pixel_data: np.ndarray, window_size: int) -> np.ndarray:
        ...
        for pixel_idx in prange(n_pixels):
            for frame_idx in range(n_frames):
                ...
                detrended[pixel_idx, frame_idx] = (
                    pixel_data[pixel_idx, frame_idx] - moving_avg
                )
    ...
    start_time = time.time()
    detrended_pixels = detrend_parallel(
        pixels_time_series.astype(np.float32),
        window_size,
    )
    detrended_stack = detrended_pixels.T.reshape(n_frames, height, width)
    detrended_stack -= np.min(detrended_stack)
